...
export default function Reports(){
  ...
  return <><AppHeader/><main className="shell"><section className="list-head"><div><p className="eyebrow">REPORT LIST</p><h1>報告書一覧</h1><p>過去の報告書を検索・確認・修正できます。</p></div><Link className="primary-link" href="/reports/new/basic">＋ 新規作成</Link></section><section className="report-overview">
      <div><span>登録件数</span><b>{rows.length}</b><small>TOTAL REPORTS</small></div>
      <div><span>完了</span><b>{rows.filter(r=>r[9]==='完').length}</b><small>COMPLETED</small></div>
      <div><span>作成中</span><b>{rows.filter(r=>r[9]!=='完').length}</b><small>IN PROGRESS</small></div>
